...
class Scanner:
    """
    · 类Scanner的对象为词法分析器提供单个字符和更加强大
      的功能：如字符回退、换行、记录行号、向后查看等。
    · 设计Scanner类的目的是分担词法分析器的工作，使词法
      分析器专注于字符识别工作，而不是字符定位工作。
    · 类Scanner的中文名字扫描器易与词法分析器的别名扫描器
      混淆，本文档中的扫描器一律指代类Scanner。
    · 类Scanner的灵感来源于Java API中的LineNUmberReader,
      其某些方法是针对词法分析器设计的，在其他应用场景并不适
      用，这是我第一次使用Python写比较长的代码，有些语法糖使
      用的并不熟练，甚至犯了将切片用 ',' 分隔的低级错误。
      self.__index__有些设计缺陷，但对词法分析器来说有省去了
      许多'+1'或'-1'的计算。
    · 注：Scanner仅测试了核心函数。
    """
